...
| `key_<sequence>`  | Мгновенный переход к закладке по указанной последовательности клавиш |
...
- `plugin whoosh key_<последовательность>` - последовательность внутри аргумента, например `key_k`, `key_<Space>`, `key_bb`.
